...
public interface TriggeredTradeSetupRepository extends JpaRepository<TriggeredTradeSetupEntity, Long> {
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE triggered_trade_setups SET exit_order_id = :exitOrderId, exit_order_placed_at = :placedAt WHERE id = :id", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query("UPDATE TriggeredTradeSetupEntity t SET t.status = :status, t.exitPrice = :exitPrice, t.exitedAt = :exitedAt  WHERE t.id = :id")
    ...
    @Query("select t from TriggeredTradeSetupEntity t where lower(t.symbol) = lower(:symbol) and t.status in :statuses")
    List<TriggeredTradeSetupEntity> findBySymbolIgnoreCaseAndStatusIn(@Param("symbol") String symbol,
    ...
    @Query("""
            ...
            WHERE LOWER(TRIM(t.source)) = LOWER(:source)
              AND (
            ...
            ORDER BY t.id
            """)
    List<TriggeredTradeSetupEntity> findBySourceForBacktestDate(@Param("source") String source,
    ...
    @Query("""
            ...
            """)
    List<TriggeredTradeSetupEntity> findBySourceForBacktestRange(@Param("source") String source,
                                                                 @Param("start") java.time.LocalDateTime start,
                                                                 @Param("end") java.time.LocalDateTime end);
