...
public interface ScriptMasterRepository extends JpaRepository<ScriptMasterEntity, Integer> {
    ...
    @Query("SELECT s.expiry FROM ScriptMasterEntity s WHERE UPPER(s.tradingSymbol) = UPPER(:symbol) AND ABS(s.strikePrice - :strikePrice) < 0.001 AND s.optionType = :optionType")
    ...
    List<ScriptMasterEntity> findByExchangeIgnoreCaseAndTradingSymbolIgnoreCase(String exchange, String tradingSymbol);
