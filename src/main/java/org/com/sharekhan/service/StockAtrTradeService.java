...
public class StockAtrTradeService {
    ...
    private static final double STOP_LOSS_ATR_MULTIPLIER = 2.0d;
    ...
    public TriggerRequest buildTriggerRequest(StockAtrTradeRequest request) {
        ...
        double stopLoss = "LONG".equals(direction)
                ? entry - (STOP_LOSS_ATR_MULTIPLIER * atr)
                : entry + (STOP_LOSS_ATR_MULTIPLIER * atr);
        double target1 = "LONG".equals(direction)
                ? entry + (TARGET1_ATR_MULTIPLIER * atr)
                : entry - (TARGET1_ATR_MULTIPLIER * atr);
        double target2 = "LONG".equals(direction)
                ? entry + (TARGET2_ATR_MULTIPLIER * atr)
                : entry - (TARGET2_ATR_MULTIPLIER * atr);
        double target3 = "LONG".equals(direction)
                ? entry + (TARGET3_ATR_MULTIPLIER * atr)
                : entry - (TARGET3_ATR_MULTIPLIER * atr);
        ...
        log.info("📐 ATR stock trade computed | stock={} spotScrip={} direction={} optionType={} entry={} atrPeriod={} atr5m={} stopLoss={} target1={} target2={} target3={} atmReference=spotEntry atmReferencePrice={} strike={} expiry={}",
                stock, spot.getScripCode(), direction, optionType, roundPrice(entry), ATR_PERIOD,
                atr, roundPrice(stopLoss), roundPrice(target1), roundPrice(target2), roundPrice(target3),
    ...
    public StockAtrTradeResponse buildResponse(TriggerRequest triggerRequest, String direction) {
        return StockAtrTradeResponse.builder()
                ...
                .atr(Math.abs(triggerRequest.getTarget1() - triggerRequest.getEntryPrice()) / TARGET1_ATR_MULTIPLIER)
