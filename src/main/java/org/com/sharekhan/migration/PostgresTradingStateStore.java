...
public class PostgresTradingStateStore {
 ...
 private <T> void batch(String table,String columns,String values,List<T> rows){if(rows.isEmpty())return;jdbc.batchUpdate("INSERT INTO "+table+" ("+columns+") OVERRIDING SYSTEM VALUE VALUES ("+values+") ON CONFLICT (id) DO NOTHING",rows.stream().map(BeanPropertySqlParameterSource::new).toArray(SqlParameterSource[]::new));}
